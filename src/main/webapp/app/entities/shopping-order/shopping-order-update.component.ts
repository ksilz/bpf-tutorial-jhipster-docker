...
export class ShoppingOrderUpdateComponent implements OnInit {
  ...
  updateForm(shoppingOrder: IShoppingOrder) {
    this.editForm.patchValue({
      ...
      ordered: shoppingOrder.ordered != null ? shoppingOrder.ordered.format(DATE_TIME_FORMAT) : null,
  ...
  private createFromForm(): IShoppingOrder {
    const entity = {
      ...
      ordered: this.editForm.get(['ordered']).value != null ? moment(this.editForm.get(['ordered']).value, DATE_TIME_FORMAT) : undefined,
  ...
  protected subscribeToSaveResponse(result: Observable<HttpResponse<IShoppingOrder>>) {
    result.subscribe((res: HttpResponse<IShoppingOrder>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
  }
