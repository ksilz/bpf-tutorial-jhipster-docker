...
describe('Product e2e test', () => {
  ...
  it('should create and save Products', async () => {
    ...
    expect(await productUpdatePage.getPictureInput()).to.endsWith(
      fileNameToUpload,
      'Expected Picture value to be end with ' + fileNameToUpload
    );
    expect(await productUpdatePage.getSpecificationInput()).to.endsWith(
      fileNameToUpload,
  ...
  it('should delete last Product', async () => {
    ...
    expect(await productDeleteDialog.getDialogTitle()).to.eq('mySimpleShopApp.product.delete.question');
